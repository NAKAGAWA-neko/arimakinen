...
function loadAndDisplayArimaCSV(csvFilePath, containerId) {
  fetch(csvFilePath)
    ...
    .then((csvString) => {
      ...
      // データ行を生成
      let previousFirstColumnValue = null;
      data.forEach((row, rowIndex) => {
        let rowHTML = `<tr>`;
        ...
        Object.entries(row).forEach(([key, cell], columnIndex) => {
          ...
          // 1列目のセル統合処理
          if (columnIndex === 0) {
            if (cell !== previousFirstColumnValue) {
              rowHTML += `<td class="${columnClass} ${cellClass}" rowspan="2">${cell}</td>`;
              previousFirstColumnValue = cell;
            }
          } else {
